...
class GetAllPrayerRequestsInteractor:
    def __init__(self, storage: UserDB, response: GetAllPrayerRequestsResponse):
        self.storage = storage
        self.response = response

    def get_all_prayer_requests_interactor(self, limit: int = 10, offset: int = 0, current_user_id: str = None) -> Response:
        try:
            if limit < 1:
            ...
            result = self.storage.get_all_prayer_requests(limit=limit, offset=offset, current_user_id=current_user_id)
